...
public class TextImage implements TextWatcher, OnLongClickListener {
	...
	private boolean _onLongClickState = false;		//if component in long click (don't delete selection if word is underline)
	private CharSequence _textToInsert = "";			//if it's not empty ("") - the Del was pressed and icon was deleted - this field can't be null, use "" instead
	...
	public void init()
	{
		...
		_textToInsert = "";			//if it's not empty ("") - the Del was pressed and icon was deleted - this field can't be null, use "" instead
		_lastChangeInx = 0;
	...
	public void onTextChanged(CharSequence s, int start, int before, int count) {
		...
		if (_textSpansEffacted != null)
		{
			...
			{
				_etMessage.getText().setSpan(_textSpansEffacted[i], start+count, _textSpansEffactedEnd[i] + (count-before), 0);
			}
			_textSpansEffacted = null;
			_textSpansEffactedEnd = null;
		}
		...
		if (_lastAction == Action.DEL_LINK || _lastAction == Action.PASTE || _lastAction == Action.DEL_CHAR)
		{
			if (_colorSpansEffacted != null)
			{
				for (int i=0; i<_colorSpansEffacted.length; i++)
				{
					...
					s.removeSpan(_colorSpansEffacted[i]);				//MUST be before doing subSeq (of currLineStrat and of currLineEnd)
	 ...
	 * 	toSend		- if the text is for sending text - replace html tags to symbols, else - replace symbols to html tags
	 */
	...
	public static StringBuilder changeLinkstate(StringBuilder input, boolean removeTags, boolean toSymbols)
	{
		...
		for (int j=1; j < HTML_COLOR_TAG.length; j++)
		{
			...
			{
				input.replace(i, i+HTML_COLOR_TAG[j][toSearch].length(), (removeTags? "":HTML_COLOR_TAG[j][toAdd]));
				i = input.indexOf(HTML_COLOR_TAG[0][toSearch], i);
				input.replace(i, i+HTML_COLOR_TAG[0][toSearch].length(), (removeTags? "":HTML_COLOR_TAG[0][toAdd]));
			}
		}
			...
			if (searchForLinks)
			{
				...
				if ( searchForEndTag) 
				{
					if ( (endLink = words[i].indexOf(HTML_COLOR_TAG[0][1])) != -1 )
					{
						...
						final String linkToAdd = text.substring(startLink + 2, endLink + textOffsetFix);	//to get only the word without the symbols
						...
						if (findEndTag == 1)
						{
							...
							res.setSpan(new ForegroundColorSpan(GOOGLE_YT_COLOR), resLinkOffsetFix, endLink, Spanned.SPAN_INTERMEDIATE);
						}
						else if (findEndTag == 2)
						{
							...
							res.setSpan(new ForegroundColorSpan(WAZE_COLOR), resLinkOffsetFix, endLink, Spanned.SPAN_INTERMEDIATE);
						}
		...
		for (i=0; i<text.length(); i++)
		{
					...
					(i < (text.length()-1)) )
			{
						...
						(text.charAt(i+1) == '}' ) )
				{
					...
					final String linkToAdd = text.substring(startLink+2, i);	//to get only the word without the symbols	
					...
					if (findEndTag == 1)
					{
						...
						res.setSpan(new ForegroundColorSpan(GOOGLE_YT_COLOR), startSpan, res.length(), Spanned.SPAN_INTERMEDIATE);
					}
					else if (findEndTag == 2)
					{
						ClickableSpan csWazeLink = new ClickableSpan() {
						...
						res.setSpan(new ForegroundColorSpan(WAZE_COLOR), startSpan, res.length(), Spanned.SPAN_INTERMEDIATE);
					}
	...
	private CharSequence addImageToCharSeq(String imageName)
	{
        ...
        ImageGetter imageGetter = new ImageGetter() {
            public Drawable getDrawable(String source) {
                ...
                d.setBounds(0, 0, (int)(d.getIntrinsicWidth()/wiegth), (int)(d.getIntrinsicHeight()/hiegth));//wantedIcon.weight, wantedIcon.height);//d.getIntrinsicWidth(), d.getIntrinsicHeight());
                return d;
            }
        };
	...
	{
		return (chkFirstChar == 'á' || chkFirstChar == 'ä' || chkFirstChar == 'å' || chkFirstChar == 'ì' || chkFirstChar == 'î');
	}
	...
	private CharSequence getMoreText(String word)
	{
		...
		res.append(tmp);	res.insert(1, (char)118);
